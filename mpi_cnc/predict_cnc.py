...
service_processes = ['./timelimit', './iglucose', './cadical130', './march_cu']
solvers = ['./kissat']
#solvers = ['./cryptominisat5.7.1', './cadical130', './v3', './kissat', './cube-glucose-min10sec-cad130.sh', './cube-glucose-min1min-cad130.sh', './cube-cad130-min10sec-cad130.sh', './cube-cad130-min1min-cad130.sh']
sh_solvers = [s for s in solvers if '.sh' in s]
SIMP_MIN_TIME_LIMIT = 60
...
def clean_garbage():
	logging.info('killing service processes')
	for process in service_processes:
		sys_str = 'killall -9 ' + process
...
def generate_random_values(template_cnf_name : str):
	lst_values_all_vars = []
	sys_str = ENUM_SOL_SOLVER + ' ' + template_cnf_name + ' --maxsol=' + str(IN_OUT_PAIRS_NUM)
	o = os.popen(sys_str).read()
	sat_blocks = o.split('s SATISFIABLE\n')
	for s_b in sat_blocks:
		lines = s_b.split('\n')
		values = []
		s = ''
		for x in lines:
			...
			if x[0] == 'v':
				s = x.split('v ')[1]
				int_vals = [int(word) for word in s.split(' ') if word != '']
				for iv in int_vals:
					if iv != 0:
						values.append(1 if iv>0 else 0)
		if len(values) > 0:
			lst_values_all_vars.append(values)
	return lst_values_all_vars
...
def make_cnf_known_sat_cube(n : int, cnf_name : str, io_vars_values : list, original_data_val : tuple):
	...
	if isSat:
		print('*** SAT was found on the minimization phase')
		print(o)
		exit(1)
...
def read_template_cnf(template_cnf_name : str):
	global cnf_vars_number
	global template_cnf_clauses
...
if __name__ == '__main__':
	...
	for n in original_data_dict:
		...
		while len(results[n]) < RANDOM_SAMPLE_SIZE:
			...
			pool.apply_async(solve_cnf_id, args=(n, solvers, template_cnf_name, cnf_id, original_data_dict[n], lst_values_all_vars[cnf_id]), callback=collect_result)
